...
// a petición el usuario que accesa
    ...
    private synchronized static Usuario LoadUsuario(final String strclUsrApp)
    {
          ...
          if (UsuarioI==null)
          {
             ResultSet rs=null;
             try{
                 System.out.println("select U.clUsrApp, U.Nombre from ADM_cUsrApp U where U.clUsrApp="+strclUsrApp);
                 strSql.append("select U.clUsrApp, U.Nombre from ADM_cUsrApp U where U.clUsrApp= ").append(strclUsrApp);
...
/*    public static List getFiltros(String strclPagina)
    ...
    public static StringBuffer getstrMenus (String strclUsrApp)
    {
           ...
           ResultSet rs=null;
           strSql.append("Select MenuConfig, coalesce(Menus,'') Menus from tmkgcA_cUsrApp where clUsrApp=").append(strclUsrApp);
           rs = UtileriasBDF.rsSQLNP( strSql.toString());
           ...
           try{
             if (rs.next())
             {
               ...
               if (rs.getString("MenuConfig").compareToIgnoreCase("0")==0){
                 ...
                 MenuIke.doMenuIke(strclUsrApp,strMenus);
                 strSql.append("set quoted_identifier off Update tmkgcA_cUsrApp set MenuConfig=1, Menus = \"").append(strMenus).append("\" where clUsrApp=").append(strclUsrApp);
