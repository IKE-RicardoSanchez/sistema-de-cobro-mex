...
<html>
    ...
    <body class="cssBody">
        ...
        StringBuffer SQL = new StringBuffer();
                    SQL.append("st_TMKGCA_CargaComboBMX");
                //System.out.append("inicio");
                ResultSet query= UtileriasBDF.rsSQLNP(SQL.toString());

                    while(query.next())
                        {
                            complemento= query.getString("ComboBmx").toString();
        ...
        StrSql.append("st_TMKGCA_ReporteBMX '").append(dsproy).append("','").append(fechaI).append("','").append(fechaF).append("',1");
            System.out.println(StrSql.toString());
        ...
        StrSql.append("st_TMKGCA_ReporteBMX '").append(dsproy).append("','").append(fechaI).append("','").append(fechaF).append("',3");
        ResultSet rs= UtileriasBDF.rsSQLNP(StrSql.toString());
        String detalleCobro ="";

        if(rs.next()){
            detalleCobro = rs.getString("Descripcion").toString();
        }
                            ...
                            <p>Periodo de<font color="blue"> <%=fechaI%> a <%=fechaF%></font> .</p>
                               ...
                               %>
                                ...
                                <table border="0" class="btm">
                                    <tr>
                                            <td><center><h4 class="VerdeValidacion">Detalle de cobro</h4></center></td>
                                    </tr>
                                    <%
                                    if(!StrSalida.toString().contains("Debes informar Fecha Inicio, Fecha Fin y Proyecto")){
                                ...
                                <%
                                    ...
                                    StrSql.append("st_TMKGCA_ReporteBMX '").append(dsproy).append("','").append(fechaI).append("','").append(fechaF).append("',2"); 
                                    UtileriasBDF.rsTableNP(StrSql.toString(), StrSalida);
                                %>   
                                    
                                <table border="0" class="btm">
                                    <tr>
                                            <td><center><h4 class="VerdeValidacion">Detalle de cancelaciones</h4></center></td>
                                    </tr>
                                     <%
                                    if(!StrSalida.toString().contains("Debes informar Fecha Inicio, Fecha Fin y Proyecto")){
                 ...
                 {  val=2;      //alert('ReportePrincipal.jsp?menu='+val+'&fechai='+fechai+'&fechaf='+fechaf+'&periodo='+periodo+'');
                    if(inicio>fin)
                        { alert('Favor de validar el periodo... \n(Fecha inicio mayor a Fecha fin.)');
                             ...
                             location.href='ReporteBmx.jsp?fechai='+fechai+'&fechaf='+fechaf+'&periodo='+periodo+'&dsproy='+dsproyecto+''; }
                 }
            }//function generaRep
             <%--
